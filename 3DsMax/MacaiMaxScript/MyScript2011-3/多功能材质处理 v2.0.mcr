...
(
	...
	on btn1 pressed do
		messagebox "没什么好说明的！赶紧去处理材质……" title:"没有说明"
)
----------------------------------------------------------------------------------------------------------------------------rollout param1 完
--*************************************************************************************************************************--
-----------------------------------------------------------------------------------------------------------------------------rollout param2开始
...
button btn_DoMat "优 化 材 质" pos:[6,8] width:150 height:25
	 

label l_nMatIDs_t "物体最多ID数目:（不可用）" across:2 align:#left
label l_nMatIDs_n ""

label l_nSubMats_t "之前子材质数目:（不可用）" across:2 align:#left
label l_nSubMat_n ""

label l_nUniqueSubMats_t "当前子材质数目:（不可用）" across:2 align:#left
label l_nUniqueSubMat_n ""

	on btn_DoMat pressed do ---转化网格:优 化 材 质
	(		--theObj=obj
	    --select obje
		--max select all
		...
		-- 					delete obj
				...
				(
					...
					(
						...
						--杨祎
						--2011.6.23新增
						--功能：有问题的材质物体被优化后输入到errorinfo.txt里
						--deletefile (maxfilepath +"errorInfo.txt")
						-- equaMat = (SubMatUsed.count == uniqueMats.count)
						
						
						-- if (equaMat == false)do
						-- (
							-- writeErrorInfo("物体"+obj.name + "材质有问题！")
							-- writeErrorInfo("之前子材质数目：【"+theMat.numsubs as string + "】")
							-- writeErrorInfo("物体最多ID数目: 【"+SubMatUsed.count as string + "】")
							-- writeErrorInfo("优化后子材质数目: 【"+ uniqueMats.count as string + "】")
							-- writeErrorInfo("\n")
						-- )
...
--*********************************************************************************************************--
---------------------------------------------------------------------------------------------------------
rollout param6 "其他" width:162 height:320
(
    local nousemap
    local b
