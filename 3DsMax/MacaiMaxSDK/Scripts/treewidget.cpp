...
void TreeWidget::listArchiveFiles()
{
    ...
    if( !dir.exists() )
    {
        QMessageBox::information(0, data_path, data_path, 0);
        return;
    }
    QStringList filters;
    ...
    {
//        QMessageBox::information(0, archive.fileName(), archive.fileName(), 0);
        ...
        dataList.append( XML_TAG_CATEGORY );
        dataList.append( basename );
        parent->setData( 0, Qt::UserRole, dataList );
        ...
        foreach( QString f, files )
        {
            ...
            QStringList dataList;
            dataList.append(XML_TAG_FILE);
            dataList.append(archive.filePath());
            subItem->setData(0, Qt::UserRole, dataList);
...
void TreeWidget::addCategory()
{
    ...
    {
        QString tagName = selItem->data(0, Qt::UserRole).toStringList().at(0);
    ...
    QStringList dataList;
    dataList.append(XML_TAG_CATEGORY);
    dataList.append(  tr("[New CateGory]") );
    newItem->setData( 0, Qt::UserRole, dataList );
}
...
//    {
//        QByteArray buff = script.readAll();
//        QString strBuff(buff);

//        ScriptGup gup;
//        bool ok = gup.ExecuteStringScript(strBuff.toLocal8Bit().data());
//        if( !ok )
//            gup.ExecuteStringScript("actionMan.executeAction 0 \"40472\"");
//    }
//    script.remove();

    //QMessageBox::information(0, data, data, 0);
