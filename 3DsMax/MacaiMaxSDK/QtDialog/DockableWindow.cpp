...
void DockableWindow::ResizeFrameToContent()
{
	...
	MoveWindow( m_hwndCuiFrame, r.left, r.top, m_widget->width(), m_widget->height(), TRUE );
}
...
void DockableWindow::ResizeContentToFrame()
{
	...
	m_widget->resize(width-10, height-30);
}
...
{
	HWND h = ::CreateCUIFrameWindow(::GetCOREInterface()->GetMAXHWnd(), name, 0, 0, 0, 0);
	if (!h) return NULL;
