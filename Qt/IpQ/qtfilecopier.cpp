...
struct CopyFileNode : public ChainNode {
    ...
    bool handle() {
        ...
        while (1) {
            ...
            qint64 in = sourceFile.read(block, 65536);
            if (in == 0) {
...
void QtFileCopierPrivate::setState(QtFileCopier::State s)
{
    ...
    /*Ìí¼Ó:QtFileCopier::Completed;*/
    else if(s == QtFileCopier::Completed)
    {
        progressTimer->stop();
        error = false;
    }
...
void QtFileCopierPrivate::copyFinished(int id, bool err)
{
    ...
    requests.remove(id);
    if (requests.isEmpty()) {
        setState(QtFileCopier::Completed);//Idle
        emit q->done(error);
    }
}
...
void QtFileCopierPrivate::copyCanceled()
{
    ...
    requests.clear();
    emit q->canceled();
    setState(QtFileCopier::Completed);
    emit q->done(false);
}
...
/**
 * @brief QtFileCopierPrivate::copyFileDestinations
 * @param sourceFile
 ...
 * @return
 */
QList<int> QtFileCopierPrivate::copyFileDestinations(const QString &sourceFile, const QStringList &destinationDirs,
                                                     QtFileCopier::CopyFlags flags, bool move)
{
    QMap<int, CopyRequest> resultList;
    QFileInfo fis(sourceFile);
    if (!fis.isDir())
    {
        QStringListIterator it(destinationDirs);
        while (it.hasNext())
        {
            QString desDir = it.next();
            QFileInfo fid(desDir);
            if (fid.isDir())
            {
                CopyRequest r = prepareRequest( true, fis.filePath(), desDir,
                                flags, move, false);
                requests[idCounter] = r;
                resultList[idCounter] = r;
                idCounter++;
...
QList<int> QtFileCopier::copyFileDestinations(const QString &sourceFile, const QStringList &destinationDirs, CopyFlags flags)
{
    if (state() != QtFileCopier::Idle)
        return QList<int>();
    return d_ptr->copyFileDestinations(sourceFile, destinationDirs, flags, false);
}
