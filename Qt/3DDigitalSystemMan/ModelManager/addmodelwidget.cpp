...
void AddModelWidget::initializWindow()
{
    ...
    QLabel *lbFile = new QLabel(tr("文件:"),this);
...
void AddModelWidget::OnOkAddModelWidgetSlot()
{
    ...
    {
        QMessageBox::information(this, tr("提示"), tr("请指定\"模型文件\""), QMessageBox::Ok);
        return;
    }
    if(m_modelType.isEmpty())
    {
        QMessageBox::information(this, tr("提示"), tr("请选择\"模型分类\""), QMessageBox::Ok);
        return;
    }
...
bool AddModelWidget::eventFilter(QObject *obj, QEvent *event)
{
    ...
    case QEvent::Enter:
    {
        ...
        {
            QToolTip::showText(QCursor::pos(), tr("支持拖拽max文件添加"), this);
        }
    };break;
